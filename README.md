...
- **依赖锁定策略**：提交和部署必须使用 `package-lock.json`，确保依赖版本可重现并避免供应链漂移。
- **CI 安全扫描**：在持续集成中运行 `npm audit --production`，或接入 Snyk 等等效方案，阻断高危依赖上线。
- **Notion Integration 权限**：仅授予读取目标数据库所需的最小权限，不使用多余的 Workspace 权限，确保令牌泄漏风险最小化。
- **运行时版本要求**：与当前配置保持一致，使用 Next.js `16.0.3`，Node.js 需满足 Next.js 官方要求的 `>=18.18.0`，以保证构建和运行行为一致。
